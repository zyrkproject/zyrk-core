...
The RPM spec file provided here is for Zyrk-Core 1.0.0 and builds on CentOS
...
reference the release tarball available from https://zyrk.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Zyrk git repository](https://github.com/zyrk/zyrk) but are not part of
...
Currently only `Source10` is used, for the example `zyrk.conf` file.
...
in the zyrk git repository. At present only an SVG version of the Zyrk
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the zyrk
...
needed when 0.12.1 is released, a proper fix is already in the Zyrk git
master branch.
...
idea to build Zyrk Core against a static version of that library compiled
...
The `zyrk.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `zyrk-server` package. At this point, most RPM based
...
The `zyrk-server` package should have SELinux support. How to properly do
...
Zyrk-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/zyrk/). He has been
packaging Zyrk for Fedora at least since 2012.
...
4. I renamed the `zyrk` package that contains the Qt GUI to `zyrk-core` as
that appears to be how the general population refers to it, in contrast to
`zyrk-xt` or `zyrk-classic`. I wanted to make sure the general population

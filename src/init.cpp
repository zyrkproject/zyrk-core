...
#include "masternode/flat-database.h"
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/zyrkproject/zyrk-core>";
    const std::string URL_WEBSITE = "<https://zyrk.io>";
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if ZYRK is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if ZYRK is started while in a temporary directory.\n",
    ...
    boost::filesystem::path pathVersionSetting = GetDataDir()/"zyrkversion.dat";
    ...
    LogPrintf("AppInitMain(): Wallet version %s logged to zyrkversion.dat", versionArg.second);
    ...
    LogPrintf("fMasterNode = %s\n", fMasterNode);
    LogPrintf("masternodeConfig.getCount(): %s\n", masternodeConfig.getCount());
    ...
    if (fMasterNode) {
        ...
        if (!strMasternodePrivKey.empty()) {
            if (!darkSendSigner.GetKeysFromSecret(strMasternodePrivKey, activeMasternode.keyMasternode,
                                                  activeMasternode.pubKeyMasternode))
                return InitError(_("Invalid masternode privkey. Please see documenation."));

            LogPrintf("pubKeyMasternode: %s\n", CBitcoinAddress(activeMasternode.pubKeyMasternode.GetID()).ToString());
        } else {
            return InitError(
                        _("You must specify a masternode privkey in the configuration. Please see documentation for help."));
        }
    }

    LogPrintf("Using Masternode config file %s\n", GetMasternodeConfigFile().string());

    if (gArgs.GetBoolArg("-masternodeconflock", true) && vpwallets.front() && (masternodeConfig.getCount() > 0)) {
    ...
    // ********************************************************* Step 11c: update block tip in zyrk modules
    ...
    // ********************************************************* Step 11d: start masternode thread

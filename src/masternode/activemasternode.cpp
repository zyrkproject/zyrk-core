...
void CActiveMasternode::ManageState() {
    ...
    if (Params().NetworkIDString() != CBaseChainParams::REGTEST && !masternodeSync.IsBlockchainSynced()) {
        ...
        //LogPrint("CActiveMasternode::ManageState -- %s: %s\n", GetStateString(), GetStatus());
        return;
    }
...
std::string CActiveMasternode::GetStatus() const {
    switch (nState) {
        ...
        case ACTIVE_MASTERNODE_INPUT_TOO_NEW:
            return strprintf("Masternode input must have at least %d confirmations",
...
bool CActiveMasternode::SendMasternodePing() {
    ...
    if (!mnodeman.Has(vin)) {
        ...
        //LogPrint("CActiveMasternode::SendMasternodePing -- %s: %s\n", GetStateString(), strNotCapableReason);
        return false;
    }

    CMasternodePing mnp(vin);
    if (!mnp.Sign(keyMasternode, pubKeyMasternode)) {
        //LogPrint("CActiveMasternode::SendMasternodePing -- ERROR: Couldn't sign Masternode Ping\n");
        return false;
    }

    // Update lastPing for our masternode in Masternode list
    if (mnodeman.IsMasternodePingedWithin(vin, MASTERNODE_MIN_MNP_SECONDS, mnp.sigTime)) {
        //LogPrint("CActiveMasternode::SendMasternodePing -- Too early to send Masternode Ping\n");
        return false;
    }

    mnodeman.SetMasternodeLastPing(vin, mnp);

    //LogPrint("CActiveMasternode::SendMasternodePing -- Relaying ping, collateral=%s\n", vin.ToString());
    mnp.Relay();
...
void CActiveMasternode::ManageStateInitial() {
    ...
    if (!fListen) {
        ...
        //LogPrint("CActiveMasternode::ManageStateInitial -- %s: %s\n", GetStateString(), strNotCapableReason);
        return;
    }
    ...
    //LogPrint("CActiveMasternode::ManageStateInitial -- Checking inbound connection to '%s'\n", service.ToString());
    //TODO
    ...
    if (!vpwallets.front()) {
        //LogPrint("CActiveMasternode::ManageStateInitial -- %s: Wallet not available\n", GetStateString());
        return;
    }

    if (vpwallets.front()->IsLocked()) {
        //LogPrint("CActiveMasternode::ManageStateInitial -- %s: Wallet is locked\n", GetStateString());
        return;
    }

    if (vpwallets.front()->GetBalance() < MASTERNODE_COIN_REQUIRED * COIN) {
        //LogPrint("CActiveMasternode::ManageStateInitial -- %s: Wallet balance is < 7500 ZYRK\n", GetStateString());
        return;
    }
    ...
    if (vpwallets.front()->GetMasternodeVinAndKeys(vin, pubKeyCollateral, keyCollateral)) {
...
void CActiveMasternode::ManageStateRemote() {
    ...
    if (infoMn.fInfoValid) {
        if (infoMn.nProtocolVersion != PROTOCOL_VERSION) {
            ...
            //LogPrint("CActiveMasternode::ManageStateRemote -- %s: %s\n", GetStateString(), strNotCapableReason);
            return;
        }
        ...
        if (nState != ACTIVE_MASTERNODE_STARTED) {
            //LogPrint("CActiveMasternode::ManageStateRemote -- STARTED!\n");
            vin = infoMn.vin;
            service = infoMn.addr;
            fPingerEnabled = true;
...
void CActiveMasternode::ManageStateLocal() {
    ...
    if (vpwallets.front()->GetMasternodeVinAndKeys(vin, pubKeyCollateral, keyCollateral)) {
        ...
        if (nInputAge < Params().GetConsensus().nMasternodeMinimumConfirmations) {
            ...
            //LogPrint("CActiveMasternode::ManageStateLocal -- %s: %s\n", GetStateString(), strNotCapableReason);
            return;
        }
        ...
        std::string strError;
        if (!CMasternodeBroadcast::Create(vin, service, keyCollateral, pubKeyCollateral, keyMasternode,
                                     pubKeyMasternode, strError, mnb)) {
            nState = ACTIVE_MASTERNODE_NOT_CAPABLE;
        ...
        //LogPrint("CActiveMasternode::ManageStateLocal -- Update Masternode List\n");
        mnodeman.UpdateMasternodeList(mnb);
        mnodeman.NotifyMasternodeUpdates();

        //send to all peers
        //LogPrint("CActiveMasternode::ManageStateLocal -- Relay broadcast, vin=%s\n", vin.ToString());
        mnb.RelayMasterNode();
    }
}

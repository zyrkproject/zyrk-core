...
void CDarksendPool::ProcessMessage(CNode *pfrom, std::string &strCommand, CDataStream &vRecv) {
    ...
    if (strCommand == NetMsgType::DSACCEPT) {
        ...
        if (!fMasterNode) {
            //LogPrint("DSACCEPT -- not a Masternode!\n");
        ...
        if (dsq.fReady) {
            ...
            if ((CNetAddr) pSubmittedToMasternode->addr != (CNetAddr) pmn->addr) {
                //LogPrint("DSQUEUE -- message doesn't match current Masternode: pSubmittedToMasternode=%s, addr=%s\n", pSubmittedToMasternode->addr.ToString(), pmn->addr.ToString());
                return;
            }
        ...
        if (!fMasterNode) {
            //LogPrint("DSVIN -- not a Masternode!\n");
            PushStatus(pfrom, STATUS_REJECTED, ERR_NOT_A_MN);
            return;
        }
        ...
        if (fMasterNode) {
            // //LogPrint("DSSTATUSUPDATE -- Can't run on a Masternode!\n");
            return;
        }

        if (!pSubmittedToMasternode) return;
        if ((CNetAddr) pSubmittedToMasternode->addr != (CNetAddr) pfrom->addr) {
            ////LogPrint("DSSTATUSUPDATE -- message doesn't match current Masternode: pSubmittedToMasternode %s addr %s\n", pSubmittedToMasternode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (!fMasterNode) {
            //LogPrint("DSSIGNFINALTX -- not a Masternode!\n");
            return;
        }
        ...
        if (fMasterNode) {
            // //LogPrint("DSFINALTX -- Can't run on a Masternode!\n");
            return;
        }

        if (!pSubmittedToMasternode) return;
        if ((CNetAddr) pSubmittedToMasternode->addr != (CNetAddr) pfrom->addr) {
            ////LogPrint("DSFINALTX -- message doesn't match current Masternode: pSubmittedToMasternode %s addr %s\n", pSubmittedToMasternode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (fMasterNode) {
            // //LogPrint("DSCOMPLETE -- Can't run on a Masternode!\n");
            return;
        }
...
void CDarksendPool::CommitFinalTransaction() {
    ...
    if (!mapDarksendBroadcastTxes.count(hashTx)) {
        ...
        CDarksendBroadcastTx dstx(tempRef, activeMasternode.vin, GetAdjustedTime());
        dstx.Sign();
...
bool CDarksendPool::SendDenominate(const std::vector <CTxIn> &vecTxIn, const std::vector <CTxOut> &vecTxOut) {
    if (fMasterNode) {
        //LogPrint("CDarksendPool::SendDenominate -- PrivateSend from a Masternode is not supported currently.\n");
        return false;
    }
    ...
    if (!nSessionID) {
        //LogPrint("CDarksendPool::SendDenominate -- No Masternode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return false;
    }
...
bool CDarksendPool::CheckPoolStateUpdate(PoolState nStateNew, int nEntriesCountNew, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID, int nSessionIDNew) {
    ...
    if (nStatusUpdate == STATUS_REJECTED) {
        //LogPrint("CDarksendPool::CheckPoolStateUpdate -- entry is rejected by Masternode\n");
        UnlockCoins();
        SetNull();
        SetState(POOL_STATE_ERROR);
...
bool CDarksendPool::SignFinalTransaction(const CTransaction &finalTransactionNew, CNode *pnode) {
    ...
    //LogPrint("CDarksendPool::SignFinalTransaction -- pushing sigs to the masternode, finalMutableTransaction=%s", finalMutableTransaction.ToString());
...
//             (float) nBalanceDenominated / COIN);
    ...
    if (nLiquidityProvider || fUseQueue) {
        ...
        BOOST_FOREACH(CDarksendQueue & dsq, vecDarksendQueue)
        {
            ...
            CMasternode *pmn = mnodeman.Find(dsq.vin);
            if (pmn == NULL) {
                //LogPrint("CDarksendPool::DoAutomaticDenominating -- dsq masternode is not in masternode list, masternode=%s\n", dsq.vin.prevout.ToStringShort());
                continue;
            }
            ...
            //LogPrint("CDarksendPool::DoAutomaticDenominating -- attempt to connect to masternode from queue, addr=%s\n", pmn->addr.ToString());
            // connect to Masternode and submit the queue request

            CNode *pnode = (pnodeFound && pnodeFound->fMasternode) ? pnodeFound : g_connman->ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
            if (pnode) {
    ...
    while (nTries < 10) {
        ...
        if (pmn == NULL) {
            //LogPrint("CDarksendPool::DoAutomaticDenominating -- Can't find random masternode!\n");
        ...
        if (pmn->nLastDsq != 0 && pmn->nLastDsq + nMnCountEnabled / 5 > mnodeman.nDsqCount) {
            //LogPrint("CDarksendPool::DoAutomaticDenominating -- Too early to mix on this masternode!"
        ...
        //LogPrint("CDarksendPool::DoAutomaticDenominating -- attempt %d connection to Masternode %s\n", nTries, pmn->addr.ToString());
        CNode *pnode = (pnodeFound && pnodeFound->fMasternode) ? pnodeFound : g_connman->ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
        if (pnode) {
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn &txin, const CPubKey &pubkey) {
    ...
    if (GetTransaction(txin.prevout.hash, txRef, Params().GetConsensus(), hash, true)) {
        ...
        if (out.nValue == MASTERNODE_COIN_REQUIRED * COIN && out.scriptPubKey == payee) return true;
    }
...
bool CDarksendQueue::CheckSignature(const CPubKey &pubKeyMasternode) {
    ...
    if (!darkSendSigner.VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
        //LogPrint("CDarksendQueue::CheckSignature -- Got bad Masternode queue signature: %s; error: %s\n", ToString(), strError);
        return false;
    }

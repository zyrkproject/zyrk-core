...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, txLockRequest.vin) {
        ...
        if(it != mapLockedOutpoints.end()) {
            ...
            //LogPrint("CInstantSend::ProcessTxLockRequest -- WARNING: Found conflicting completed Transaction Lock, skipping current one, txid=%s, completed lock txid=%s\n",
    ...
    if(!CreateTxLockCandidate(txLockRequest)) {
        // smth is not right
        //LogPrint("CInstantSend::ProcessTxLockRequest -- CreateTxLockCandidate failed, txid=%s\n", txHash.ToString());
        return false;
    }
    //LogPrint("CInstantSend::ProcessTxLockRequest -- accepted, txid=%s\n", txHash.ToString());
...
bool CInstantSend::CreateTxLockCandidate(const CTxLockRequest& txLockRequest)
{
    ...
    if(itLockCandidate == mapTxLockCandidates.end()) {
        //LogPrint("CInstantSend::CreateTxLockCandidate -- new, txid=%s\n", txHash.ToString());
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasMasternodeVoted(itOutpointLock->first, activeMasternode.vin.prevout)) {
                    ...
                    // skip it anyway
                    fAlreadyVoted = true;
                    //LogPrint("CInstantSend::Vote -- WARNING: We already voted for this outpoint, skipping: txHash=%s, outpoint=%s\n",
        ...
        if(!vote.Sign()) {
            //LogPrint("CInstantSend::Vote -- Failed to sign consensus vote\n");
            return;
        }
        if(!vote.CheckSignature()) {
            //LogPrint("CInstantSend::Vote -- Signature invalid\n");
            return;
        }
        ...
        if(itOutpointLock->second.AddVote(vote)) {
            //LogPrint("CInstantSend::Vote -- Vote created successfully, relaying: txHash=%s, outpoint=%s, vote=%s\n",
            ...
            if(itVoted == mapVotedOutpoints.end()) {
                ...
                if(mapVotedOutpoints[itOutpointLock->first].size() > 1) {
                    ...
                    //LogPrint("CInstantSend::Vote -- WARNING: Vote conflicts with some existing votes: txHash=%s, outpoint=%s, vote=%s\n",
                         //   txHash.ToString(), itOutpointLock->first.ToStringShort(), nVoteHash.ToString());
                }
            }
        ...
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                if(it2->second.HasMasternodeVoted(vote.GetOutpoint(), vote.GetMasternodeOutpoint())) {
                    ...
                    // pose ban score too.
                    //LogPrint("CInstantSend::ProcessTxLockVote -- masternode sent conflicting votes! %s\n", vote.GetMasternodeOutpoint().ToStringShort());
                    return false;
                }
            }
        }
...
bool CInstantSend::ResolveConflicts(const CTxLockCandidate& txLockCandidate, int nMaxBlocks)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, txLockCandidate.txLockRequest.vin) {
        ...
        if(GetLockedOutPointTxHash(txin.prevout, hashConflicting) && txHash != hashConflicting) {
            ...
            if(itLockCandidate == mapTxLockCandidates.end() || itLockCandidateConflicting == mapTxLockCandidates.end()) {
                ...
                //LogPrint("CInstantSend::ResolveConflicts -- ERROR: Found conflicting completed Transaction Lock, but one of txLockCandidate-s is missing, txid=%s, conflicting txid=%s\n",
                     //   txHash.ToString(), hashConflicting.ToString());
                return false;
            }
            //LogPrint("CInstantSend::ResolveConflicts -- WARNING: Found conflicting completed Transaction Lock, dropping both, txid=%s, conflicting txid=%s\n",
                //    txHash.ToString(), hashConflicting.ToString());
            ...
            //LogPrint("CInstantSend::ResolveConflicts -- ERROR: Failed to complete Transaction Lock, conflicts with mempool, txid=%s\n", txHash.ToString());
            return false;
        }
    } // FOREACH
    ...
    BOOST_FOREACH(const CTxIn& txin, txLockCandidate.txLockRequest.vin) {
        ...
        if(!GetUTXOCoin(txin.prevout, coin)) {
            ...
            //LogPrint("CInstantSend::ResolveConflicts -- ERROR: Failed to find UTXO %s, can't complete Transaction Lock\n", txin.prevout.ToStringShort());
            return false;
        }
    }
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itLockCandidate != mapTxLockCandidates.end()) {
        ...
        if(txLockCandidate.IsExpired(pCurrentBlockIndex->nHeight)) {
            //LogPrint("CInstantSend::CheckAndRemove -- Removing expired Transaction Lock Candidate: txid=%s\n", txHash.ToString());
            std::map<COutPoint, COutPointLock>::iterator itOutpointLock = txLockCandidate.mapOutPointLocks.begin();
            while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
...
bool CTxLockVote::IsValid(CNode* pnode) const
{
    ...
    if(!CheckSignature()) {
        //LogPrint("CTxLockVote::IsValid -- Signature invalid\n");
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    if(!infoMn.fInfoValid) {
        //LogPrint("CTxLockVote::CheckSignature -- Unknown Masternode: masternode=%s\n", outpointMasternode.ToString());
        return false;
    }

    if(!darkSendSigner.VerifyMessage(infoMn.pubKeyMasternode, vchMasternodeSignature, strMessage, strError)) {
        //LogPrint("CTxLockVote::CheckSignature -- VerifyMessage() failed, error: %s\n", strError);
        return false;
    }

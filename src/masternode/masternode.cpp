...
void CMasternode::Check(bool fForce) {
    ...
    if (IsPoSeBanned()) {
        ...
        //LogPrint("CMasternode::Check -- Masternode %s is unbanned and back in list now\n", vin.prevout.ToStringShort());
        DecreasePoSeBanScore();
    } else if (nPoSeBanScore >= MASTERNODE_POSE_BAN_MAX_SCORE) {
        nActiveState = MASTERNODE_POSE_BAN;
        // ban for the whole payment cycle
        nPoSeBanHeight = nHeight + mnodeman.size();
        //LogPrint("CMasternode::Check -- Masternode %s is banned till block %d now\n", vin.prevout.ToStringShort(), nPoSeBanHeight);
        return;
    }
    ...
    if (!fWaitForPing || fOurMasternode) {
        ...
        bool fWatchdogActive = masternodeSync.IsSynced(chainActive.Height()) && mnodeman.IsWatchdogActive();
...
std::string CMasternode::ToString() const {
    ...
    str += " ";
    str += std::to_string(lastPing == CMasternodePing() ? sigTime : lastPing.sigTime);
    str += " ";
    str += std::to_string(lastPing == CMasternodePing() ? 0 : lastPing.sigTime - sigTime);
    str += " ";
...
void CMasternode::UpdateLastPaid(const CBlockIndex *pindex, int nMaxBlocksToScanBack) {
    if (!pindex) {
        //LogPrint("CMasternode::UpdateLastPaid pindex is NULL\n");
        return;
    }
...
//        //LogPrint("mnpayments.mapMasternodeBlocks.count(BlockReading->nHeight)=%s\n", mnpayments.mapMasternodeBlocks.count(BlockReading->nHeight));
//        //LogPrint("mnpayments.mapMasternodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2)=%s\n", mnpayments.mapMasternodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2));
...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string &strErrorRet, CMasternodeBroadcast &mnbRet, bool fOffline) {
    //LogPrint("CMasternodeBroadcast::Create\n");
    CTxIn txin;
    ...
    if (!fOffline && !masternodeSync.IsBlockchainSynced()) {
        ...
        //LogPrint("CMasternodeBroadcast::Create -- %s\n", strErrorRet);
        return false;
    }
    ...
    if (!vpwallets.front()->GetMasternodeVinAndKeys(txin, pubKeyCollateralAddressNew, keyCollateralAddressNew, strTxHash, strOutputIndex)) {
    ...
    if (!Lookup(strService.c_str(), addr, mainnetDefaultPort, false)) {
        return InitError(strprintf(_("CMasternodeBroadcast Create(): Invalid masternode broadcast: '%s'"), strService));
    }

    if (Params().NetworkIDString() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != mainnetDefaultPort) {
            strErrorRet = strprintf("Invalid port %u for masternode %s, only %d is supported on mainnet.", addr.GetPort(), strService, mainnetDefaultPort);
            //LogPrint("CMasternodeBroadcast::Create -- %s\n", strErrorRet);
            return false;
        }
    } else if (addr.GetPort() == mainnetDefaultPort) {
        strErrorRet = strprintf("Invalid port %u for masternode %s, %d is the only supported on mainnet.", addr.GetPort(), strService, mainnetDefaultPort);
        //LogPrint("CMasternodeBroadcast::Create -- %s\n", strErrorRet);
        return false;
    }

    return Create(txin, addr, keyCollateralAddressNew, pubKeyCollateralAddressNew, keyMasternodeNew, pubKeyMasternodeNew, strErrorRet, mnbRet);
}
...
bool CMasternodeBroadcast::SimpleCheck(int &nDos) {
    ...
    if (!IsValidNetAddr()) {
        //LogPrint("CMasternodeBroadcast::SimpleCheck -- Invalid addr, rejected: masternode=%s  addr=%s\n",
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        //LogPrint("CMasternodeBroadcast::SimpleCheck -- Signature rejected, too far into the future: masternode=%s\n", vin.prevout.ToStringShort());
        nDos = 1;
        return false;
    }
    ...
    if (nProtocolVersion < mnpayments.GetMinMasternodePaymentsProto()) {
        //LogPrint("CMasternodeBroadcast::SimpleCheck -- ignoring outdated Masternode: masternode=%s  nProtocolVersion=%d\n", vin.prevout.ToStringShort(), nProtocolVersion);
        return false;
    }
    ...
    if (pubkeyScript.size() != 25) {
        //LogPrint("CMasternodeBroadcast::SimpleCheck -- pubKeyCollateralAddress has the wrong size\n");
        nDos = 100;
        return false;
    }
    ...
    if (pubkeyScript2.size() != 25) {
        //LogPrint("CMasternodeBroadcast::SimpleCheck -- pubKeyMasternode has the wrong size\n");
        nDos = 100;
        return false;
    }

    if (!vin.scriptSig.empty()) {
        //LogPrint("CMasternodeBroadcast::SimpleCheck -- Ignore Not Empty ScriptSig %s\n", vin.ToString());
        nDos = 100;
        return false;
    }
...
bool CMasternodeBroadcast::Update(CMasternode *pmn, int &nDos) {
    ...
    if (pmn->sigTime > sigTime) {
        //LogPrint("CMasternodeBroadcast::Update -- Bad sigTime %d (existing broadcast is at %d) for Masternode %s %s\n",
    ...
    if (pmn->IsPoSeBanned()) {
        //LogPrint("CMasternodeBroadcast::Update -- Banned by PoSe, masternode=%s\n", vin.prevout.ToStringShort());
        return false;
    }

    // IsVnAssociatedWithPubkey is validated once in CheckOutpoint, after that they just need to match
    if (pmn->pubKeyCollateralAddress != pubKeyCollateralAddress) {
        //LogPrint("CMasternodeBroadcast::Update -- Got mismatched pubKeyCollateralAddress and vin\n");
        nDos = 33;
        return false;
    }

    if (!CheckSignature(nDos)) {
        //LogPrint("CMasternodeBroadcast::Update -- CheckSignature() failed, masternode=%s\n", vin.prevout.ToStringShort());
        return false;
    }
    ...
    if (!pmn->IsBroadcastedWithin(MASTERNODE_MIN_MNB_SECONDS) || (fMasterNode && pubKeyMasternode == activeMasternode.pubKeyMasternode)) {
        ...
        //LogPrint("CMasternodeBroadcast::Update -- Got UPDATED Masternode entry: addr=%s\n", addr.ToString());
...
bool CMasternodeBroadcast::CheckOutpoint(int &nDos) {
    ...
    if (!CheckSignature(nDos)) {
        //LogPrint("CMasternodeBroadcast::CheckOutpoint -- CheckSignature() failed, masternode=%s\n", vin.prevout.ToStringShort());
        return false;
    }

    {
        ...
        */
        ...
        if (coin.out.nValue != MASTERNODE_COIN_REQUIRED * COIN) {
            //LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Failed to find Masternode UTXO, masternode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
        if (chainActive.Height() - coin.nHeight + 1 < Params().GetConsensus().nMasternodeMinimumConfirmations) {
            //LogPrint("CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO must have at least %d confirmations, masternode=%s\n",
    ...
    if (!darkSendSigner.IsVinAssociatedWithPubkey(vin, pubKeyCollateralAddress)) {
        //LogPrint("CMasternodeMan::CheckOutpoint -- Got mismatched pubKeyCollateralAddress and vin\n");
        nDos = 33;
        return false;
    }

    // verify that sig time is legit in past
    // should be at least not earlier than block when 10000 ZYRK tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex *pMNIndex = (*mi).second; // block for 10k ZYRK tx -> 1 confirmation
            CBlockIndex *pConfIndex = chainActive[pMNIndex->nHeight + Params().GetConsensus().nMasternodeMinimumConfirmations - 1]; // block where tx got nMasternodeMinimumConfirmations
            if (pConfIndex->GetBlockTime() > sigTime) {
                //LogPrint("CMasternodeBroadcast::CheckOutpoint -- Bad sigTime %d (%d conf block is at %d) for Masternode %s %s\n",
...
bool CMasternodeBroadcast::Sign(CKey &keyCollateralAddress) {
    ...
    if (!darkSendSigner.SignMessage(strMessage, vchSig, keyCollateralAddress)) {
        //LogPrint("CMasternodeBroadcast::Sign -- SignMessage() failed\n");
        return false;
    }

    if (!darkSendSigner.VerifyMessage(pubKeyCollateralAddress, vchSig, strMessage, strError)) {
        //LogPrint("CMasternodeBroadcast::Sign -- VerifyMessage() failed, error: %s\n", strError);
        return false;
    }
...
bool CMasternodeBroadcast::CheckSignature(int &nDos) {
    ...
    if (!darkSendSigner.VerifyMessage(pubKeyCollateralAddress, vchSig, strMessage, strError)) {
        //LogPrint("CMasternodeBroadcast::CheckSignature -- Got bad Masternode announce signature, error: %s\n", strError);
        nDos = 100;
        return false;
    }
...
void CMasternodeBroadcast::RelayMasterNode() {
    //LogPrint("CMasternodeBroadcast::RelayMasterNode\n");
...
bool CMasternodePing::Sign(CKey &keyMasternode, CPubKey &pubKeyMasternode) {
    ...
    if (!darkSendSigner.SignMessage(strMessage, vchSig, keyMasternode)) {
        //LogPrint("CMasternodePing::Sign -- SignMessage() failed\n");
        return false;
    }

    if (!darkSendSigner.VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
        //LogPrint("CMasternodePing::Sign -- VerifyMessage() failed, error: %s\n", strError);
        return false;
    }
...
bool CMasternodePing::CheckSignature(CPubKey &pubKeyMasternode, int &nDos) {
    ...
    if (!darkSendSigner.VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
        //LogPrint("CMasternodePing::CheckSignature -- Got bad Masternode ping signature, masternode=%s, error: %s\n", vin.prevout.ToStringShort(), strError);
        nDos = 33;
        return false;
    }
    return true;
...
bool CMasternodePing::SimpleCheck(int &nDos) {
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        //LogPrint("CMasternodePing::SimpleCheck -- Signature rejected, too far into the future, masternode=%s\n", vin.prevout.ToStringShort());
        nDos = 1;
        return false;
    }
...
bool CMasternodePing::CheckAndUpdate(CMasternode *pmn, bool fFromNewBroadcast, int &nDos) {
...

    {
        ...
        if ((*mi).second && (*mi).second->nHeight < chainActive.Height() - 24) {
            //LogPrint("CMasternodePing::CheckAndUpdate -- Masternode ping is invalid, block hash is too old: masternode=%s  blockHash=%s\n", vin.prevout.ToStringShort(), blockHash.ToString());
            // nDos = 1;
            return false;
        }
    }
    ...
    // and update masternodeman.mapSeenMasternodeBroadcast.lastPing which is probably outdated

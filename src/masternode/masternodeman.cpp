...
CMasternodeMan::CMasternodeMan() : cs(),
  vMasternodes(),
...
void CMasternodeMan::AskForMN(CNode* pnode, const CTxIn &vin)
{
    ...
    if (it1 != mWeAskedForMasternodeListEntry.end()) {
        ...
        if (it2 != it1->second.end()) {
            ...
            //LogPrint("CMasternodeMan::AskForMN -- Asking same peer %s for missing masternode entry again: %s\n", pnode->addr.ToString(), vin.prevout.ToStringShort());
        } else {
            // we already asked for this outpoint but not this node
            //LogPrint("CMasternodeMan::AskForMN -- Asking new peer %s for missing masternode entry: %s\n", pnode->addr.ToString(), vin.prevout.ToStringShort());
        }
    } else {
        // we never asked any node for this outpoint
        //LogPrint("CMasternodeMan::AskForMN -- Asking peer %s for missing masternode entry for the first time: %s\n", pnode->addr.ToString(), vin.prevout.ToStringShort());
    }
...
void CMasternodeMan::CheckAndRemove()
{
    ...
    //LogPrint("CMasternodeMan::CheckAndRemove\n");
...
//                it->FlagGovernanceItemsAsDirty();
                ...
                bool fAsk = pCurrentBlockIndex &&
                            ...
                            masternodeSync.IsSynced(chainActive.Height()) &&
    ...
    }
    {
        ...
        //LogPrint("CMasternodeMan::CheckAndRemove -- %s\n", ToString());
    ...
    if(Params().NetworkIDString() == CBaseChainParams::MAIN) {
        if(!(pnode->addr.IsRFC1918() || pnode->addr.IsLocal())) {
            ...
            if(it != mWeAskedForMasternodeList.end() && GetTime() < (*it).second) {
                //LogPrint("CMasternodeMan::DsegUpdate -- we already asked %s for the list; skipping...\n", pnode->addr.ToString());
                return;
            }
        }
    }
...
char* CMasternodeMan::GetNotQualifyReason(CMasternode& mn, int nBlockHeight, bool fFilterSigTime, int nMnCount)
{
    ...
    if (mn.nProtocolVersion < mnpayments.GetMinMasternodePaymentsProto()) {
        ...
        // //LogPrint("mnpayments.GetMinMasternodePaymentsProto=%s!\n", mnpayments.GetMinMasternodePaymentsProto());
        char* reasonStr = new char[256];
...
CMasternode* CMasternodeMan::GetNextMasternodeInQueueForPayment(int nBlockHeight, bool fFilterSigTime, int& nCount)
{
    ...
    */
    ...
    BOOST_FOREACH(CMasternode &mn, vMasternodes)
    {
        ...
        if (reasonStr != NULL) {
            //LogPrint("masternodeman", "Masternode, %s, addr(%s), qualify %s\n",
...
CMasternode* CMasternodeMan::FindRandomNotInVec(const std::vector<CTxIn> &vecToExclude, int nProtocolVersion)
{
    ...
    //LogPrint("CMasternodeMan::FindRandomNotInVec -- %d enabled masternodes, %d masternodes to choose from\n", nCountEnabled, nCountNotExcluded);
...
CMasternode* CMasternodeMan::GetMasternodeByRank(int nRank, int nBlockHeight, int nMinProtocol, bool fOnlyActive)
{
    ...
    if(!GetBlockHash(blockHash, nBlockHeight)) {
        //LogPrint("CMasternode::GetMasternodeByRank -- ERROR: GetBlockHash() failed at nBlockHeight %d\n", nBlockHeight);
        return NULL;
    }
    ...
    g_connman->ForEachNode([](CNode* pnode){
        if(!(darkSendPool.pSubmittedToMasternode != NULL && pnode->addr == darkSendPool.pSubmittedToMasternode->addr))
            if(pnode->fMasternode) {
                // //LogPrint("Closing Masternode connection: peer=%d, addr=%s\n", pnode->GetId(), pnode->addr.ToString());
...
//    //LogPrint("masternode", "CMasternodeMan::ProcessMessage, strCommand=%s\n", strCommand);
    ...
    if (strCommand == NetMsgType::MNANNOUNCE) { //Masternode Broadcast
        ...
        //LogPrint("MNANNOUNCE -- Masternode announce, masternode=%s\n", mnb.vin.prevout.ToStringShort());
        ...
        BOOST_FOREACH(CMasternode& mn, vMasternodes) {
            ...
            if (vin == mn.vin) {
                //LogPrint("DSEG -- Sent 1 Masternode inv to peer %d\n", pfrom->GetId());
                return;
            }
        }
        ...
        if(vin == CTxIn()) {
            ...
            g_connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::SYNCSTATUSCOUNT, MASTERNODE_SYNC_LIST, nInvCount));
            //LogPrint("DSEG -- Sent %d Masternode invs to peer %d\n", nInvCount, pfrom->GetId());
            return;
        }
...
{
    if(!masternodeSync.IsSynced(chainActive.Height()) || vMasternodes.empty()) return;
    ...
    BOOST_FOREACH(CMasternode* pmn, vBan) {
        //LogPrint("CMasternodeMan::CheckSameAddr -- increasing PoSe ban score for masternode %s\n", pmn->vin.prevout.ToStringShort());
...
bool CMasternodeMan::SendVerifyRequest(const CAddress& addr, const std::vector<CMasternode*>& vSortedByAddr)
{
    ...
    if(pnode == NULL) {
        //LogPrint("CMasternodeMan::SendVerifyRequest -- can't connect to node to verify it, addr=%s\n", addr.ToString());
        return false;
    }
    ...
    //LogPrint("CMasternodeMan::SendVerifyRequest -- verifying node using nonce %d addr=%s\n", mnv.nonce, addr.ToString());
    const CNetMsgMaker msgMaker(pnode->GetSendVersion());
...
//        // peer should not ask us that often
        //LogPrint("MasternodeMan::SendVerifyReply -- ERROR: peer already asked me recently, peer=%d\n", pnode->GetId());
    ...
    if(!GetBlockHash(blockHash, mnv.nBlockHeight)) {
        //LogPrint("MasternodeMan::SendVerifyReply -- can't get block hash for unknown block height %d, peer=%d\n", mnv.nBlockHeight, pnode->GetId());
        return;
    }
    ...
    if(!darkSendSigner.SignMessage(strMessage, mnv.vchSig1, activeMasternode.keyMasternode)) {
        //LogPrint("MasternodeMan::SendVerifyReply -- SignMessage() failed\n");
        return;
    }
    ...
    if(!darkSendSigner.VerifyMessage(activeMasternode.pubKeyMasternode, mnv.vchSig1, strMessage, strError)) {
        //LogPrint("MasternodeMan::SendVerifyReply -- VerifyMessage() failed, error: %s\n", strError);
        return;
    }
...
void CMasternodeMan::ProcessVerifyReply(CNode* pnode, CMasternodeVerification& mnv)
{
    ...
    if(!netfulfilledman.HasFulfilledRequest(pnode->addr, strprintf("%s", NetMsgType::MNVERIFY)+"-request")) {
        //LogPrint("CMasternodeMan::ProcessVerifyReply -- ERROR: we didn't ask for verification of %s, peer=%d\n", pnode->addr.ToString(), pnode->GetId());
        Misbehaving(pnode->GetId(), 20);
        return;
    }
    ...
    if(mWeAskedForVerification[pnode->addr].nonce != mnv.nonce) {
        //LogPrint("CMasternodeMan::ProcessVerifyReply -- ERROR: wrong nounce: requested=%d, received=%d, peer=%d\n",
    ...
    if(mWeAskedForVerification[pnode->addr].nBlockHeight != mnv.nBlockHeight) {
        //LogPrint("CMasternodeMan::ProcessVerifyReply -- ERROR: wrong nBlockHeight: requested=%d, received=%d, peer=%d\n",
    ...
    if(!GetBlockHash(blockHash, mnv.nBlockHeight)) {
        ...
        //LogPrint("MasternodeMan::ProcessVerifyReply -- can't get block hash for unknown block height %d, peer=%d\n", mnv.nBlockHeight, pnode->GetId());
        return;
    }

//    // we already verified this address, why node is spamming?
    if(netfulfilledman.HasFulfilledRequest(pnode->addr, strprintf("%s", NetMsgType::MNVERIFY)+"-done")) {
        //LogPrint("CMasternodeMan::ProcessVerifyReply -- ERROR: already verified %s recently\n", pnode->addr.ToString());
        Misbehaving(pnode->GetId(), 20);
        return;
    }

    {
            ...
            if(CAddress(it->addr, NODE_NETWORK) == pnode->addr) {
                if(darkSendSigner.VerifyMessage(it->pubKeyMasternode, mnv.vchSig1, strMessage1, strError)) {
                    ...
                    if(!darkSendSigner.SignMessage(strMessage2, mnv.vchSig2, activeMasternode.keyMasternode)) {
                        //LogPrint("MasternodeMan::ProcessVerifyReply -- SignMessage() failed\n");
                        return;
                    }
                    ...
                    if(!darkSendSigner.VerifyMessage(activeMasternode.pubKeyMasternode, mnv.vchSig2, strMessage2, strError)) {
                        //LogPrint("MasternodeMan::ProcessVerifyReply -- VerifyMessage() failed, error: %s\n", strError);
                        return;
                    }
        ...
        if(!prealMasternode) {
            ...
            //LogPrint("CMasternodeMan::ProcessVerifyReply -- ERROR: no real masternode found for addr %s\n", pnode->addr.ToString());
            Misbehaving(pnode->GetId(), 20);
            return;
        }
        //LogPrint("CMasternodeMan::ProcessVerifyReply -- verified real masternode %s for addr %s\n",
        ...
        //LogPrint("CMasternodeMan::ProcessVerifyBroadcast -- PoSe score increased for %d fake masternodes, addr %s\n",
...
void CMasternodeMan::ProcessVerifyBroadcast(CNode* pnode, const CMasternodeVerification& mnv)
{
    ...
    if(!GetBlockHash(blockHash, mnv.nBlockHeight)) {
        // this shouldn't happen...
        //LogPrint("MasternodeMan::ProcessVerifyBroadcast -- Can't get block hash for unknown block height %d, peer=%d\n", mnv.nBlockHeight, pnode->GetId());
        return;
    }
...

    {
        ...
        if(!pmn1) {
            //LogPrint("CMasternodeMan::ProcessVerifyBroadcast -- can't find masternode1 %s\n", mnv.vin1.prevout.ToStringShort());
            return;
        }

        CMasternode* pmn2 = Find(mnv.vin2);
        if(!pmn2) {
            //LogPrint("CMasternodeMan::ProcessVerifyBroadcast -- can't find masternode2 %s\n", mnv.vin2.prevout.ToStringShort());
            return;
        }

        if(pmn1->addr != mnv.addr) {
            //LogPrint("CMasternodeMan::ProcessVerifyBroadcast -- addr %s do not match %s\n", mnv.addr.ToString(), pnode->addr.ToString());
            return;
        }

        if(darkSendSigner.VerifyMessage(pmn1->pubKeyMasternode, mnv.vchSig1, strMessage1, strError)) {
            //LogPrint("MasternodeMan::ProcessVerifyBroadcast -- VerifyMessage() for masternode1 failed, error: %s\n", strError);
            return;
        }

        if(darkSendSigner.VerifyMessage(pmn2->pubKeyMasternode, mnv.vchSig2, strMessage2, strError)) {
            //LogPrint("MasternodeMan::ProcessVerifyBroadcast -- VerifyMessage() for masternode2 failed, error: %s\n", strError);
            return;
        }
        ...
        //LogPrint("CMasternodeMan::ProcessVerifyBroadcast -- verified masternode %s for addr %s\n",
        ...
        //LogPrint("CMasternodeMan::ProcessVerifyBroadcast -- PoSe score incresed for %d fake masternodes, addr %s\n",
...
void CMasternodeMan::UpdateMasternodeList(CMasternodeBroadcast mnb)
{
    try {
        ...
        PrintExceptionContinue(&e, "UpdateMasternodeList");
    }
}
    ...
    if(mnb.CheckOutpoint(nDos)) {
        ...
        if(fMasterNode && mnb.pubKeyMasternode == activeMasternode.pubKeyMasternode) {
            ...
            if(mnb.nProtocolVersion == PROTOCOL_VERSION) {
                ...
                //LogPrint("CMasternodeMan::CheckMnbAndUpdateMasternodeList -- Got NEW Masternode entry: masternode=%s  sigTime=%lld  addr=%s\n",
                            //mnb.vin.prevout.ToStringShort(), mnb.sigTime, mnb.addr.ToString());
                activeMasternode.ManageState();
            } else {
                // ... otherwise we need to reactivate our node, do not add it to the list and do not relay
                // but also do not ban the node we get this message from
                //LogPrint("CMasternodeMan::CheckMnbAndUpdateMasternodeList -- wrong PROTOCOL_VERSION, re-activate your MN: message nProtocolVersion=%d  PROTOCOL_VERSION=%d\n", mnb.nProtocolVersion, PROTOCOL_VERSION);
                return false;
            }
        }
        mnb.RelayMasterNode();
    } else {
        //LogPrint("CMasternodeMan::CheckMnbAndUpdateMasternodeList -- Rejected Masternode entry: %s  addr=%s\n", mnb.vin.prevout.ToStringShort(), mnb.addr.ToString());
        return false;
    }
...
void CMasternodeMan::UpdateLastPaid()
{
    ...
    if(fLiteMode) return;
    if(!pCurrentBlockIndex) {
        // //LogPrint("CMasternodeMan::UpdateLastPaid, pCurrentBlockIndex=NULL\n");
        return;
    }

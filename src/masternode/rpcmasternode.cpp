    ...
    if (params[0].get_str() == "start") {
        {
            if(vpwallets.front() == NULL)
                return "rpcmasternode(): Error loading wallet";
...
UniValue masternode(const JSONRPCRequest& req) {
...

    {
        ...
        if (strCommand == "list") {
            ...
            return masternodelist(JSONParams);
        }
        ...
        if (strCommand == "connect") {
            ...
            if (!Lookup(strAddress.c_str(), addr, port, false)) {
                return InitError(strprintf(_("rpcmasternode masternode(): Invalid masternode: '%s'"), strAddress));
            }
        ...
        if (strCommand == "debug") {
            ...
            if (!vpwallets.front() || !vpwallets.front()->GetMasternodeVinAndKeys(vin, pubkey, key))
        ...
        if (strCommand == "start-alias") {
            ...
            BOOST_FOREACH(CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
                if (mne.getAlias() == strAlias) {
                    ...
                    bool fResult = CMasternodeBroadcast::Create(mne.getIp(), mne.getPrivKey(), mne.getTxHash(),
    ...
    //        LogPrintf("start-alias: statusObj=%s\n", statusObj);
        ...
        if (strCommand == "list-conf") {
            ...
            BOOST_FOREACH(CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
                ...
                resultObj.push_back(Pair("masternode_" + std::to_string(i), mnObj));
                i++;
            }
        ...
        if (strCommand == "outputs") {
            ...
            vpwallets.front()->AvailableCoins(vPossibleCoins, true, NULL, 1, MAX_MONEY, MAX_MONEY, 0, 0, 0x7FFFFFFF, ONLY_7500);
...
UniValue masternodelist(const JSONRPCRequest &req) {
    UniValue params = req.params;
    bool fHelp = req.fHelp;
    ...
    if (fHelp || (
        ...
        throw std::runtime_error(
                "masternodelist ( \"mode\" \"filter\" )\n"
                        ...
                        "  qualify        - Print qualify status of a masternode based on current block\n"
...
UniValue masternodebroadcast(const JSONRPCRequest &req) {
    ...
    if (strCommand == "decode") {
        ...
        returnObj.push_back(Pair("overall", strprintf(
                "Successfully decoded broadcast messages for %d masternodes, failed to decode %d, total %d",
                nSuccessful, nFailed, nSuccessful + nFailed)));
    ...
    if (strCommand == "relay") {
        ...
        returnObj.push_back(Pair("overall", strprintf(
                "Successfully relayed broadcast messages for %d masternodes, failed to relay %d, total %d", nSuccessful,

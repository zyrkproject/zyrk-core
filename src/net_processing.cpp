...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // Masternode Related Inventory Messages
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        static const int FORK_MIN_PEER_PROTO_VERSION = nHeight >= Params().GetConsensus().nStartAnonymizeFeeDistribution ? MIN_PEER_DPOS_PROTO_VERSION : MIN_PEER_PROTO_VERSION;
        ...
        if(strSubVer == "/Zyrk Core:0.1.0.0/"){
            // disconnect from peers older than this version due to min tx fee
            LogPrint(BCLog::NET, "peer=%d using obsolete version %i; disconnecting\n", pfrom->GetId(), nVersion);
            connman->PushMessage(pfrom, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                               strprintf("Version must be %d or greater", 0100)));
            pfrom->fDisconnect = true;
            return false;
        }
    ...
    else if (strCommand == NetMsgType::TX || strCommand == NetMsgType::DSTX || strCommand == NetMsgType::TXLOCKREQUEST || strCommand == "witness-tx")
    {
        ...
        if (strCommand == NetMsgType::TXLOCKREQUEST) {
            ...
            if (pmn == NULL) {
                LogPrintf("privatesend", "DSTX -- Can't find masternode %s to verify %s\n",
                         dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return false;
            }

            if (!pmn->fAllowMixingTx) {
                LogPrintf("privatesend", "DSTX -- Masternode %s is sending too many transactions %s\n",
                         dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return true;
    ...
    else if (strCommand == NetMsgType::FILTERCLEAR)
    {
        ...
        if (found) {
            ...
            masternodeSync.ProcessMessage(pfrom, strCommandNonConst, vRecv);
        } else {
            // Ignore unknown commands for extensibility

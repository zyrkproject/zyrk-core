...
const QString AddressTableModel::AnonymizeVault = "G";
...
public:
        ...
        cachedAddressTable.clear();
        {
            ...
            BOOST_FOREACH(const CZerocoinEntry& item, listPubcoin)
            {
                if(item.randomness != 0 && item.serialNumber != 0){
                    ...
                    cachedAddressTable.append(AddressTableEntry(AddressTableEntry::AnonymizeVault,
    ...
    void updateEntry(const QString &pubCoin, const QString &isUsed, int status)
    {
        ...
        int lowerIndex = (lower - cachedAddressTable.begin());
        bool inModel = (lower != upper);
        AddressTableEntry::Type newEntryType = AddressTableEntry::AnonymizeVault;
...
QVariant AddressTableModel::data(const QModelIndex &index, int role) const
{
    ...
    else if (role == TypeRole)
    {
        ...
        case AddressTableEntry::Receiving:
            return Receive;
        case AddressTableEntry::AnonymizeVault:
            return AnonymizeVault;
        default: break;
        }
    }
    return QVariant();
...
bool AddressTableModel::anonymizeZYRK(string &stringError, string denomAmount)
{
    WalletModel::UnlockContext ctx(walletModel->requestUnlock());
    if(!ctx.isValid())
    {
...
bool AddressTableModel::convertAnonymize(string &stringError, string thirdPartyAddress, string denomAmount)
{
    WalletModel::UnlockContext ctx(walletModel->requestUnlock());
    if(!ctx.isValid())
    {

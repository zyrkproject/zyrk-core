...
#include "qt/forms/ui_masternode.h"
...
MasterNode::MasterNode(const PlatformStyle *platformStyle, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::MasterNode),
    clientModel(0),
    walletModel(0)
{
    ui->setupUi(this);
...
void MasterNode::setClientModel(ClientModel *model)
{
...
void MasterNode::setWalletModel(WalletModel *model)
{
    this->walletModel = model;
}

void MasterNode::showContextMenu(const QPoint &point)
{
    QTableWidgetItem *item = ui->tableWidgetMyMasternodes->itemAt(point);
    if(item) contextMenu->exec(QCursor::pos());
}

void MasterNode::StartAlias(std::string strAlias)
{
...
void MasterNode::StartAll(std::string strCommand)
{
...
void MasterNode::updateMyMasternodeInfo(QString strAlias, QString strAddr, const COutPoint& outpoint)
{
    ...
    masternode_info_t infoMn = mnodeman.GetMasternodeInfo(CTxIn(outpoint));
...
void MasterNode::updateMyNodeList(bool fForce)
{
...
void MasterNode::updateNodeList()
{
...
void MasterNode::on_filterLineEdit_textChanged(const QString &strFilterIn)
{
...
void MasterNode::on_startButton_clicked()
{
    std::string strAlias;
    {
        LOCK(cs_mymnlist);
...
void MasterNode::on_startAllButton_clicked()
{
    // Display message box
    QMessageBox::StandardButton retval = QMessageBox::question(this, tr("Confirm all masternode start"),
...
void MasterNode::on_startMissingButton_clicked()
{
...
void MasterNode::on_tableWidgetMyMasternodes_itemSelectionChanged()
{
...
void MasterNode::on_UpdateButton_clicked()
{
    updateMyNodeList(true);
}

...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    else if (nNet > 0 || wtx.IsCoinBase())
    {
        ...
        for(unsigned int i = 0; i < wtx.tx->vout.size(); i++)
        {
            ...
            if(mine)
            {
                //skip public anonymize key data models
                ...
                if(tempTx->IsZerocoinMint()){
                    sub.type = TransactionRecord::Anonymized;
                }
        ...
        else
        {
            ...
            if(wtx.tx->IsZerocoinSpend()){
                ...
                for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++)
                {
                    ...
                    if (ExtractDestination(txout.scriptPubKey, address))
                    {
                        sub.type = TransactionRecord::UnAnonymized;
                        sub.address = EncodeDestination(address);
    ...
    else
    {
        ...
        else if (status.depth < RecommendedNumConfirmations)
        {
            if(wtx.tx->IsZerocoinMint())
                status.status = TransactionStatus::Shading;
            else

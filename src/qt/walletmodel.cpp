...
CAmount WalletModel::getAnonymizeBalance() const
{
    return wallet->GetAnonymizeBalance();
}

CAmount WalletModel::getAnonymizeBalanceUnconfirmed() const
{
    return wallet->GetAnonymizeBalanceUnconfirmed();
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    CAmount newAnonymizeBalance = getAnonymizeBalance();
    CAmount newAnonymizeBalanceUnconfirmed = getAnonymizeBalanceUnconfirmed();
    ...
    if(cachedBalance != newBalance || cachedUnconfirmedBalance != newUnconfirmedBalance || cachedImmatureBalance != newImmatureBalance ||
        cachedWatchOnlyBalance != newWatchOnlyBalance || cachedWatchUnconfBalance != newWatchUnconfBalance ||
            cachedWatchImmatureBalance != newWatchImmatureBalance || cachedStaked != newStaked || cacheAnonymizeBalance != newAnonymizeBalance
            || cacheAnonymizeBalanceUnconfirmed != newAnonymizeBalanceUnconfirmed)
    {
        ...
        cacheAnonymizeBalance = newAnonymizeBalance;
        cacheAnonymizeBalanceUnconfirmed = newAnonymizeBalanceUnconfirmed;
        Q_EMIT balanceChanged(newBalance, newUnconfirmedBalance, newImmatureBalance,
                            newWatchOnlyBalance, newWatchUnconfBalance, newWatchImmatureBalance, newAnonymizeBalance, newAnonymizeBalanceUnconfirmed, newStaked, newWatchStakedBalance);
    }
}
        ...
        const std::string &purpose, ChangeType status)
{
    ...
    if(address.type() == typeid(CAnonymizeAddress)){
        CAnonymizeAddress sxAddr = boost::get<CAnonymizeAddress>(address);
...
static void NotifyAnonymizeProtocolChanged(WalletModel *walletmodel, CWallet *wallet,  const std::string &pubCoin, int denomination, const std::string &isUsed, ChangeType status)
{
    qDebug() << "NotifyAnonymizeProtocolChanged:" + QString::fromStdString(pubCoin) + " " + QString::fromStdString(isUsed) + " status=" + QString::number(status);
    QMetaObject::invokeMethod(walletmodel, "updateAddressBook", Qt::QueuedConnection,
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyZerocoinChanged.connect(boost::bind(NotifyAnonymizeProtocolChanged, this, _1, _2, _3, _4, _5));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyZerocoinChanged.disconnect(boost::bind(NotifyAnonymizeProtocolChanged, this, _1, _2, _3, _4, _5));
...
UniValue CallRPC(std::string args, std::string wallet)
{
    ...
    for (size_t i = 0; i < args.size(); ++i)
    {
            ...
            && (c == ' ' || c == '\t'))
        {
            ...
            vArgs.push_back(zyrk::TrimQuotes(s));
            s.clear();
            continue;
        };

...
/* Prefix used to indicate a Uzyrk socket in a FooPort configuration. */
static const char unix_socket_prefix[] = "unix:";
/* Prefix used to indicate a Uzyrk socket with spaces in it, in a FooPort
 * configuration. */
...
options_act_reversible(const or_options_t *old_options, char **msg)
{
  ...
  if (options->ControlSocket || options->ControlSocketsGroupWritable) {
    *msg = tor_strdup("Uzyrk domain sockets (ControlSocket) not supported "
                           ...
                           const int listener_type)
{
  SMARTLIST_FOREACH_BEGIN(ports, const port_cfg_t *, port) {
    ...
    if (port->is_unix_addr) {
      /* Uzyrk sockets aren't accessible over a network. */
                  ...
                  const unsigned flags)
{
  ...
  for (; ports; ports = ports->next) {
    ...
    /* Let's start to check if it's a Uzyrk socket path. */
    if (is_unix_tagged_addr) {
#ifndef HAVE_SYS_UN_H
      log_warn(LD_CONFIG, "Uzyrk sockets not supported on this system.");
      goto err;
#endif

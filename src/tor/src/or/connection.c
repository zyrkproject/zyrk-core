...
{
  /* For now only control ports or SOCKS ports can be Uzyrk domain sockets
...
connection_free_(connection_t *conn)
{
  ...
  if (!connection_is_listener(conn)) {
    ...
    if (conn->socket_family == AF_UNIX) {
      /* For now only control and SOCKS ports can be Uzyrk domain sockets
       * and listeners at the same time */
 ...
 * <b>listenaddress</b> provides the path to the Uzyrk socket.
 *
                     ...
                     socklen_t *len_out)
{
  ...
  if (strlcpy(sockaddr->sun_path, listenaddress, sizeof(sockaddr->sun_path))
    ...
    log_warn(LD_CONFIG, "Uzyrk socket path '%s' is too long to fit.",
                     ...
                     socklen_t *len_out)
{
  ...
  log_fn(LOG_ERR, LD_BUG,
         "Uzyrk domain sockets not supported, yet we tried to create one.");
  *len_out = 0;
  tor_fragile_assert();
  return NULL;
}
                               ...
                               int purpose, const port_cfg_t *port)
{
  ...
  if (check_private_dir(p, flags, options->User) < 0) {
    ...
    log_warn(LD_GENERAL, "Before Tor can create a %s in %s, the directory "
             ...
             "account that is running Tor.  (On some Uzyrk systems, anybody "

...
#if defined(NDEBUG)
# error "ZYRK cannot be compiled without assertions."
#endif
...
//TODO: configure masternode fees
...
const std::string strMessageMagic = "ZYRK Signed Message:\n";
...
bool GetMasternodeFeePayment(int64_t &returnFee, bool &payFees, const CBlock &pBlock){

    CAmount totalAnonymized = 0;
    vector<CAmount> mintVector;
    mintVector.clear();
    if(chainActive.Height() + 1 >= Params().GetConsensus().nStartAnonymizeFeeDistribution){
        //Time to payout all masternodes and check
        //LogPrintf("\nGetMasternodeFeePayment(): height=%d, modulo=%d\n",
        //          (chainActive.Height() + 1), ((chainActive.Height() + 1) % Params().GetConsensus().nAnonymizeFeeDistributionCycle));
        if(((chainActive.Height() + 1) % Params().GetConsensus().nAnonymizeFeeDistributionCycle) == 0){
            //Subtract 1 from sample since we check current block fees
            int sample = Params().GetConsensus().nAnonymizeFeeDistributionCycle - 1;
            mintVector.clear();
            ...
            for(auto ctx: pBlock.vtx){
                //Found Anonymize fee transaction
            ...
            returnFee = totalAnonymized * 0.0025;
            payFees = true;
            return true;
        }
        //Make sure all Anonymize fees in this block are not paid out
        else{
            for(auto ctx: pBlock.vtx){
                //Found Anonymize fee transaction
                if(!ctx->IsZerocoinSpend() && ctx->IsZerocoinMint()){
...
bool CheckAnonymizeProtocolFeePayouts(const CBlock &pBlock, int64_t &totalFees){
    ...
    if(masternodeSync.IsSynced(chainActive.Height())){
        ...
        vector<CScript> masternodeVectorWinners;
        masternodeVectorWinners.clear();
        ...
        for(auto node: masternodeVector){
            ...
            if(node.IsEnabled() && (node.sigTime >= ensureNodeActiveBefore)){
                ...
                masternodeVectorWinners.push_back(nodeScript);
            }
        }
        ...
        for(CTxOut out: pBlock.vtx[0]->vout){
            if((std::find(masternodeVectorWinners.begin(), masternodeVectorWinners.end(), out.scriptPubKey) != masternodeVectorWinners.end()) && out.nValue == feePayout) {
                //erase this check to prevent duplicate payouts
                masternodeVectorWinners.erase(std::find(masternodeVectorWinners.begin(), masternodeVectorWinners.end(), out.scriptPubKey));
                totalNodesPaid++;
            }
        }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!ConnectBlockAnonymize(state, chainparams, pindex, &block))
        return false;

    //Set money supply on block once PoS starts, calculate previous total
    if(!pindex->pprev->IsProofOfStake() && pindex->IsProofOfStake()){
        CAmount nTotalSupply = pindex->pprev->nHeight * GetBlockSubsidy(pindex->pprev->nHeight, block.nTime);
        pindex->nMoneySupply = nTotalSupply;
        //LogPrintf("ZYRK Money Supply: %llf", pindex->nMoneySupply/COIN);
    }
    ...
    if (fTimestampIndex)
    {
        ...
        if (!pblocktree->WriteTimestampBlockIndex(CTimestampBlockIndexKey(pindex->GetBlockHash()), CTimestampBlockIndexValue(pindex->nTime)))
...
bool CChainState::DisconnectTip(CValidationState& state, const CChainParams& chainparams, DisconnectedBlockTransactions *disconnectpool)
{
    ...
    DisconnectTipAnonymize(block, pindexDelete);

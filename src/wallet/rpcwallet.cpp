...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nReturns a new ZYRK address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "2. \"address_type\"   (string, optional) The address type to use. Options are \"masternode\", \"p2sh-segwit(default)\", and \"bech32\". Default is set by -addresstype.\n"
            "\nResult:\n"
            "\"address\"    (string) The new zyrk address\n"
            "\nExamples:\n"
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current ZYRK address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account zyrk address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new ZYRK address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The zyrk address to be associated with an account.\n"
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ZYRK address");
    }
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The zyrk address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a zyrk address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
static void SendMoney(CWallet * const pwallet, const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew, const CCoinControl& coin_control/*, CScript anonymizeKey*/)
{
    CAmount curBalance = pwallet->GetBalance();
    ...
    /*
    //include public anonymize key in transaction if using a anonymize address
    if(!anonymizeKey.empty()){
        CRecipient recipientAnonymize = {anonymizeKey, 0, fSubtractFeeFromAmount};
        vecSend.push_back(recipientAnonymize);
    }
    */
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The zyrk address to send to.\n"
            ...
            "                             The recipient will receive less zyrk than you enter in the amount field.\n"
    ...
    CTxDestination dest;
    /*
    CScript anonymizeKey;

    if (IsAnonymizeAddress(request.params[0].get_str()))
    {
        CAnonymizeAddress sxAddr;
        if (sxAddr.SetEncoded(request.params[0].get_str()))
        {
            ...
            anonymizeKey = CScript() << OP_RETURN << ephem_pubkey;
    ...
    */
    ...
    SendMoney(pwallet, dest, nAmount, fSubtractFeeFromAmount, wtx, coin_control/*, anonymizeKey*/);
...
UniValue leasestaking(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "\nLease an amount of zyrk to a certain address to stake.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
            "1. \"lease address\"                    (string, required) The zyrk address to lease stakes to.\n"
            "2. \"amount\"                              (numeric or string, required) The amount in " + CURRENCY_UNIT + " to send. eg 0.1\n"
            "3. \"fee percent\"                         (numeric, optional) The percentage to allow delegator to take. eg 11.9 (11.9%)\n"
            "4. \"lease percent reward address\"     (string, optional) The zyrk address to force lease fee stakes to.\n"
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The zyrk address\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The zyrk address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The zyrk address for transactions.\n"
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a zyrk address."
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            ...
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The zyrk address to send funds to.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The zyrk address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less zyrk than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid ZYRK address: ") + name_);
        }
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" \"address_type\" )\n"
            ...
            "Each key is a ZYRK address or hex-encoded public key.\n"
            ...
            "2. \"keys\"                         (string, required) A json array of zyrk addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"                  (string) zyrk address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            ...
            "}\n"
            "\nResult (DEPRECATED. To see this result in v0.16 instead, please start zyrkd with -deprecatedrpc=addmultisigaddress).\n"
            "        clients should transition to the new output api before upgrading to v0.17.\n"
            "\"address\"                         (string) A zyrk address associated with the keys.\n"
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("addwitnessaddress")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "addwitnessaddress is deprecated and will be fully removed in v0.17. "
            "To use addwitnessaddress in v0.16, restart zyrkd with -deprecatedrpc=addwitnessaddress.\n"
...
void ListTransactions(CWallet* const pwallet, const CWalletTx& wtx, const std::string& strAccount, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        for (const COutputEntry& s : listSent)
        {
            ...
            entry.push_back(Pair("is_anonymized", wtx.tx->IsZerocoinMint() || wtx.tx->IsZerocoinSpend()));
            ret.push_back(entry);
        }
    }
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        for (const COutputEntry& r : listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                if (fLong)
                    WalletTxToJSON(wtx, entry);
                entry.push_back(Pair("is_unanonymized", wtx.tx->IsZerocoinSpend()));
                ret.push_back(entry);
            }
        }
    }
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The zyrk address of the transaction. Not present for \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The zyrk address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "      \"address\" : \"address\",          (string) The zyrk address involved in the transaction\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending zyrk\n"
    ...
    StartShutdown();
    return "wallet encrypted; Zyrk server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending zyrk.\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"keypoololdest\": xxxxxx,         (numeric) the timestamp (seconds since Uzyrk epoch) of the oldest pre-generated key in the key pool\n"
    ...
    obj.push_back(Pair("anonymize_vault",       ValueFromAmount(pwallet->GetAnonymizeBalance())));
    obj.push_back(Pair("anonymize_vault_unconfirmed",       ValueFromAmount(pwallet->GetAnonymizeBalanceUnconfirmed())));
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of zyrk addresses to filter\n"
            "    [\n"
            "      \"address\"     (string) zyrk address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the zyrk address\n"
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid ZYRK address: ") + input.get_str());
            }
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default pool address) The zyrk address to receive the change\n"
    ...
    if (!request.params[1].isNull()) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid zyrk address");
            }
...
UniValue anonymizeamount(const JSONRPCRequest& request)
{
    CWallet *pwalletMain = GetWalletForJSONRPCRequest(request);

    if (request.fHelp || request.params.size() > 1)
        throw runtime_error("anonymizeamount <amount>(whole numbers only)\n" + HelpRequiringPassphrase(pwalletMain));

    int64_t nAmount = request.params[0].get_int64();

    bool strError = pwalletMain->AnonymizeModeMintTrigger(std::to_string(nAmount));

    if (!strError)
        throw JSONRPCError(RPC_WALLET_ERROR, "anonymizeamount");

    return "Sucessfully anonymized " + std::to_string(nAmount) +  " ZYRK";
}

UniValue unanonymizeamount(const JSONRPCRequest& request)
{
    CWallet *pwalletMain = GetWalletForJSONRPCRequest(request);

    if (request.fHelp || request.params.size() == 0 || request.params.size() > 2)
        throw runtime_error("unanonymizeamount <amount>(whole numbers only) <addresstosend>\n" + HelpRequiringPassphrase(pwalletMain));
    ...
    if (request.params.size() > 1){
        ...
        if(!IsAnonymizeAddress(toKey))
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "rpcwallet unanonymizeamount(): Invalid toKey address");
    }
    ...
    std::string strError = pwalletMain->AnonymizeModeSpendTrigger(std::to_string(nAmount), toKey);
...
UniValue mintzerocoin(const JSONRPCRequest& request)
{
    ...
    return "Sucessfully anonymized " + std::to_string(nAmount/COIN) +  " ZYRK";
}
...
UniValue totalanonymized(const JSONRPCRequest& request) {

    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
                "totalanonymized\n"
                        "Returns total amount of anonymized ZYRK on the network\n");
...
UniValue enableTor(const JSONRPCRequest& request){
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
                "enabletor <enable>(false/true)\n"
                        ...
                        "Please restart the ZYRK daemon to update your changes");
    ...
    boost::filesystem::path pathTorSetting = GetDataDir()/"zyrktorsetting.dat";
    if(fStatus){
        if (WriteBinaryFileTor(pathTorSetting.string().c_str(), "enabled")) {
            result = ("Please restart the Zyrk Core wallet to route your connection to obfuscate your IP address. \nSyncing your wallet might be slower.");
        }else{
            result = ("Obfuscation cannot enable");
        }
    }else{
        if (WriteBinaryFileTor(pathTorSetting.string().c_str(), "disabled")) {
            result = ("Please restart the Zyrk Core wallet to disable IP obfuscation.");
        } else {
...
UniValue torStatus(const JSONRPCRequest& request){
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
                "torstatus\n"
                        "Returns the status of tor obfuscation on your ZYRK daemon");
...
UniValue manageaddressbook(const JSONRPCRequest &request)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_PARAMETER, _("Invalid ZYRK address."));
...
UniValue getstakinginfo(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"moneysupply\": xxxxxxx,        (numeric) the total amount of ZYRK in the network\n"
            "  \"reserve\": xxxxxxx,            (numeric) the total amount of ZYRK in the network\n"
...
UniValue refillanonymizekeys(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error("refillanonymizekeys <amount>(default=100)\n" + HelpRequiringPassphrase(pwalletMain));
    ...
    vector<std::string> anonymizeKey;
    ...
    for(int i = listUnloadedPubcoin.size(); i < ideal; i++){
        ...
        if(newCoinTemp.getPublicCoin().validate()){
            ...
            if (!walletdb.WriteUnloadedZCEntry(zerocoinTx))
                return "anonymizekeys() Error: Only able to create " + std::to_string(i) + " keys";

            std::vector<unsigned char> commitmentKey = newCoinTemp.getPublicCoin().getValue().getvch();
            CommitmentKey pubCoin(commitmentKey);
            anonymizeKey.push_back(pubCoin.GetPubCoinDataBase58() + "-");
        }
        else
            i--;
    }
    ...
    string fullKey;
    for(string key: anonymizeKey)
        fullKey+=key;

    UniValue results(UniValue::VARR);
    results.push_back("Sucessfully created anonymizekey amount: " + fullKey);
    return results;
}
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
                "listunloadedpubcoins amount(default=all)\n"
                        "\nResults are an array of public anonymize keys:\n");
...
UniValue getpubcoinpack(const JSONRPCRequest& request) {
    ...
    if(keyAmount > listUnloadedPubcoin.size())
        throw JSONRPCError(RPC_WALLET_ERROR,
                           "Error: Not enough Commitment Keys, please run refillanonymizekeys");
...
UniValue payunloadedpubcoins(const JSONRPCRequest& request) {
    ...
    if(keyPack.IsValidPack())
        strError = pwalletMain->AnonymizeModeSpendTrigger(std::to_string(nAmount), "", keyPack.GetPubCoinPackScript());
    else
...
{ //  category              name                        actor (function)           argNames
    ...
    // Anonymize functions (experimental)
    { "Anonymize",             "listunspentanonymize",    &listunspentmintzerocoins,  {} },
    { "Anonymize",             "anonymizeamount",              &anonymizeamount,               {"amount"} },
    { "Anonymize",             "unanonymizeamount",            &unanonymizeamount,             {"amount"} },
    { "Anonymize",             "resetanonymize",          &resetmintzerocoin,         {} },
    { "Anonymize",             "setanonymizestatus",      &setmintzerocoinstatus,     {} },
    { "Anonymize",             "listanonymize",           &listmintzerocoins,         {"all"} },
    { "Anonymize",             "listpubcoins",             &listpubcoins,              {} },
    { "Anonymize",             "totalanonymize",             &totalanonymized,              {} },

    { "Anonymize",             "refillanonymizekeys",          &refillanonymizekeys,           {"amount"} },
    { "Anonymize",             "listunloadedpubcoins",     &listunloadedpubcoins,      {"amount"} },
    { "Anonymize",             "payunloadedpubcoins",      &payunloadedpubcoins,       {"amount", "address"} },
    { "Anonymize",             "getpubcoinpack",           &getpubcoinpack,            {"amount"} },
    { "Anonymize",             "resetzerocoinamounts",     &resetzerocoinamounts,      {} },
    { "Anonymize",             "resetzerocoinunconfirmed", &resetzerocoinunconfirmed,  {} },
    { "Anonymize",             "listallserials",           &listallserials,            {"height"} },
    { "Anonymize",             "eraseusedzerocoindata",    &eraseusedzerocoindata,     {""} },
    { "Anonymize",             "encryptallzerocoins",      &encryptallzerocoins,       {""} },
    { "Anonymize",             "decryptallzerocoins",      &decryptallzerocoins,       {""} },

    //TOR routing functions
    { "TOR",             "enabletor",        &enableTor,        {"set"} },

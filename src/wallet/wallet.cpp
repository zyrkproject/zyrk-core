...
#include "masternode/darksend.h"
#include "masternode/instantx.h"
...
public:
    ...
    void operator()(const CAnonymizeAddress &stxAddr) {
        CScript script;
    }
...
bool CWallet::AddToWalletIfInvolvingMe(const CTransactionRef& ptx, const CBlockIndex* pIndex, int posInBlock, bool fUpdate)
{
    const CTransaction& tx = *ptx;
    {
        ...
        bool foundZerocoin = FindUnloadedAnonymizeTransactions(tx);
...
CAmount CWallet::GetAnonymizeBalance() const
{
...
CAmount CWallet::GetAnonymizeBalanceUnconfirmed() const
{
    std::vector<COutput> vCoins;
    ...
    return nTotal - GetAnonymizeBalance();
}
...

    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                } else if (nCoinType == ONLY_NOT7500IFMN) {
                    found = !(fMasterNode && pcoin->tx->vout[i].nValue == MASTERNODE_COIN_REQUIRED * COIN);
                } else if (nCoinType == ONLY_NONDENOMINATED_NOT7500IFMN) {
                    if (IsCollateralAmount(pcoin->tx->vout[i].nValue)) continue; 
                    found = !IsDenominatedAmount(pcoin->tx->vout[i].nValue);
                    if (found && fMasterNode) found = pcoin->tx->vout[i].nValue != MASTERNODE_COIN_REQUIRED * COIN; 
                } else if (nCoinType == ONLY_7500) {
                    found = pcoin->tx->vout[i].nValue == MASTERNODE_COIN_REQUIRED * COIN;
                    isGN = true;
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (pick_new_inputs) {
                    ...
                    {
                        if (nCoinType == ONLY_NOT7500IFMN) {
                            strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 ZYRK.");
                        } else if (nCoinType == ONLY_NONDENOMINATED_NOT7500IFMN) {
                            strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 10000 ZYRK.");
...
{
    FindUnloadedAnonymizeTransactions(*wtxNew.tx);
    {
        LOCK2(cs_main, cs_wallet);
...
static const std::string OUTPUT_TYPE_STRING_MASTERNODE = "masternode";
...
OutputType ParseOutputType(const std::string& type, OutputType default_type)
{
    ...
    }  else if (type == OUTPUT_TYPE_STRING_MASTERNODE) {
        return OUTPUT_TYPE_LEGACY;
    } else {
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                //add 0.25% tx fee to all Anonymize transaction
...
//            LogPrintf("txNew:%s\n", txNew.ToString());
            ...
            if (coinId == INT_MAX){
                strFailReason = _("network privacy set too low. There needs to be at least 2 anonymized values of this type! ");
                return false;
            }
...
bool CWallet::CreateZerocoinSpendTransactionBatch(std::string &toKey, vector <CScript> pubCoinScripts, vector <int64_t> nValueBatch, vector <libzerocoin::CoinDenomination> denominationBatch,
    ...
    if(pubCoinScripts.empty()){
        ...
        else if (IsAnonymizeAddress(toKey))
        {
    ...
    for(int i = 0; i < nValueBatch.size(); i++){
        {
            LOCK2(cs_main, cs_wallet);
            {
                ...
                if (coinId == INT_MAX){
                    strFailReason = _("network privacy set too low. There needs to be at least 2 anonymized values of this type!")  + std::to_string(coinHeight) + " " + std::to_string(coinId);
                    return false;
                }
...
//unlock wallet and create anonymize timer
bool CWallet::EnableAnonymizeMode(SecureString strWalletPass, string totalAmount){

    this->NotifyZerocoinChanged.connect(boost::bind(&CWallet::NotifyAnonymizeChanged, this, _1, _2, _3, _4, _5));

    //sanity check for decimal
    if (totalAmount.find('.') != std::string::npos)
        return error("%s: Error: The Anonymize Mode value needs to be a whole number.", __func__);
    ...
    if(!AnonymizeModeMintTrigger(totalAmount))
        return error("%s: Error: Cannot trigger anonymize mode mint.", __func__);

    return true;
}

//Lock wallet and destroy anonymize timer
bool CWallet::DisableAnonymizeMode(){
    LOCK(this->cs_wallet);
    this->nRelockTime = 0;
    this->NotifyZerocoinChanged.disconnect(boost::bind(&CWallet::NotifyAnonymizeChanged, this, _1, _2, _3, _4, _5));
    this->Lock();
    return true;
}

//push mint and spends into a queue
void CWallet::NotifyAnonymizeChanged(CWallet *wallet, const std::string &pubCoin, int denomination, const std::string &isUsed, ChangeType status)
{
    ...
    if(isUsed == "New")
        AnonymizeModeSpendTrigger(std::to_string(denom));
    if(isUsed == "Used")
        AnonymizeModeMintTrigger(std::to_string(denom));
    ...
    BOOST_FOREACH(const CZerocoinEntry &minIdPubcoin, pc) {
        if(minIdPubcoin.IsUsed == false){
            ...
            if(coinHeight >= chainActive.Height() + ZEROCOIN_CONFIRM_HEIGHT){
                //if(!AnonymizeModeSpendTrigger(std::to_string(minIdPubcoin.denomination)))
...
//anonymize timer mint responder
bool CWallet::AnonymizeModeMintTrigger(string totalAmount, vector<CScript> pubCoinScripts){

    string stringError;
    //Autobackup wallet into anonymizebackups
    string backupDir = GetDataDir().string() + "/anonymizebackups/wallet-" + std::to_string(GetTime()) + ".dat";
    fs::create_directories(fs::path(GetDataDir().string() + "/anonymizebackups/"));

    //only maintain max of 20 backups
    std::string path = GetDataDir().string() + "/anonymizebackups/";
    int m = 0;
    int t = 0;
...
int AnonymizeDenom(CAmount amount){
    switch(amount){
    case CAmount(1):
        return 0;
    case CAmount(5):
...
bool ClosestDenoms(CAmount amount, int totalZerocoins, CAmount demoniationList[8][1]){
    ...
    for (int i = 0; i < totalZerocoins; i++) {
        ...
        if (amount == currentDenomination[currentDenominationIndex]) {
            ...
            if(demoniationList[AnonymizeDenom(currentDenomination[currentDenominationIndex])][0] > 0){
                demoniationList[AnonymizeDenom(currentDenomination[currentDenominationIndex])][0]--;
                amount = 0;
            }
            i = totalZerocoins;
            break;
...
//anonymize timer spend responder
std::string CWallet::AnonymizeModeSpendTrigger(string totalAmount, string toKey, vector<CScript> pubCoinScripts){
    ...
    if(!this->GetDBHandle().Backup(backupDir))
        return "AnonymizeModeSpendTrigger(): Error: Cannot create wallet backup.";
    ...
    CAmount amount = 0;
    if (!ParseFixedPoint(totalAmount, 8, &amount))
        return "AnonymizeModeSpendTrigger(): Error: Invalid amount.";
    if (!MoneyRange(amount))
        return "AnonymizeModeSpendTrigger(): Error: Amount out of range.";
    ...
    for (int i = 0; i <= totalZerocoins; i++) {
        ...
        if (amount == currentDenomination[currentDenominationIndex]) {
            //check if we have this denomination
            if(demoniationList[AnonymizeDenom(currentDenomination[currentDenominationIndex])][0] > 0){
                toSpend[AnonymizeDenom(currentDenomination[currentDenominationIndex])][0]++;
                demoniationList[AnonymizeDenom(currentDenomination[currentDenominationIndex])][0]--;
                amount = 0;
                i = totalZerocoins;
                break;
    ...
    if(amount == 0){
        ...
        if (this->IsLocked())
            return "AnonymizeModeSpendTrigger(): Error: The wallet needs to be unlocked.";

        //check if minting these spends
        if(!pubCoinScripts.empty()){
            //Not enough payout scripts
            if(pubCoinScripts.size() < denominationBatch.size())
                return "AnonymizeModeSpendTrigger(): Error: Not enough mint payout scripts "
            ...
            for(int ps = 0; ps < pubCoinScripts.size(); ps++){
                ...
                CBigNum pubCoin(vector<unsigned char>(pubCoinScripts[ps].begin()+6, pubCoinScripts[ps].end()));
                if (!zcState->CanAddMintToMempool(pubCoin)) {
                    return "AnonymizeModeSpendTrigger(): Error: key has already been used!";
                }
            }
        ...
        for(int vecSplit = 0; vecSplit < ((denominationBatch.size()/4) + 1); vecSplit++){
            ...
            if(!CreateZerocoinSpendModelBatch(stringError, denominationBatchSub, toKey, pubCoinScriptsSub)){
                if(vecSplit > 0){
                    CAmount amountAnonymized = 0;
                    for (int x = 0; x < vecSplit; x++){
                        amountAnonymized += currentDenomination[x];
                        amountAnonymized += currentDenomination[x + 1];
                        amountAnonymized += currentDenomination[x + 2];
                        amountAnonymized += currentDenomination[x + 3];
                    }
                    return "AnonymizeModeSpendTrigger(): Error: Was only able to unanonymize %s ZYRK - %s." + std::to_string(amountAnonymized) + stringError;
                }
                else
                    return "AnonymizeModeSpendTrigger(): Error: Failed to unanonymize anonymized ZYRK - %s." +  stringError;
            }
        ...
        return "Sucessfully sent " + totalAmount + " anonymized ZYRK";
    }
    else {
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector <CTxIn> &vecTxInRet, CAmount &nValueRet,
    ...
    vector <COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, MAX_MONEY, MAX_MONEY,0, 0, 9999999, nPrivateSendRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT7500IFMN : ONLY_DENOMINATED);
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        ...
        if (fMasterNode && out.tx->tx->vout[out.i].nValue == MASTERNODE_COIN_REQUIRED * COIN) continue; //masternode input
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn &txinRet, CPubKey &pubKeyRet, CKey &keyRet, std::string strTxHash,
    ...
    AvailableCoins(vPossibleCoins, true, NULL, false, MAX_MONEY, MAX_MONEY,0, 0, 9999999, ONLY_7500);
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if (fAnonymizable) {
                ...
                if (fMasterNode && wtx.tx->vout[i].nValue == MASTERNODE_COIN_REQUIRED * COIN) continue;
...
size_t CWallet::CountColdstakeOutputs()
{
    ...
    AvailableCoins(vAvailableCoins, false, &coinControl, nMinimumAmount, nMaximumAmount, nMinimumSumAmount, nMaximumCount, nMinDepth, nMaxDepth, ONLY_NONDENOMINATED_NOT7500IFMN);
...
void CWallet::AvailableCoinsForStaking(std::vector<COutput> &vCoins, int64_t nTime, int nHeight) const
{
...

    {
        ...
        int coinbaseMaturity = nHeight >= Params().GetConsensus().nStartAnonymizeFeeDistribution ? COINBASE_MATURITY_V2 : COINBASE_MATURITY;
...
bool CWallet::CreateCoinStake(unsigned int nBits, int64_t nTime, int nBlockHeight, int64_t nFees, CMutableTransaction &txNew, CKey &key, CBlockTemplate *pblocktemplate, int64_t nAnonymizeFees, std::vector<unsigned char> &commitment, uint256 witnessroot)
{
    ...
    for (; it != setCoins.end(); ++it)
    {
        ...
        if (CheckKernel(pindexPrev, nBits, nTime, prevoutStake, &nBlockTime))
        {
            ...
            */
            ...
            txNew.nVersion = ZYRK_TXN_VERSION;
    ...
    CScript DEV_SCRIPT;
    {
      DEV_SCRIPT = GetScriptForDestination(DecodeDestination(""));
    }

    txNew.vout.push_back(CTxOut(DEVELOPMENT_REWARD_POST_POS * GetBlockSubsidy(chainActive.Height(), nTime), CScript(DEV_SCRIPT.begin(), DEV_SCRIPT.end())));


    CBlock *pblock = &pblocktemplate->block; // pointer for convenience


    //payout 10 masternodes for rewards
    if(chainActive.Height() + 1 < Params().GetConsensus().nStartAnonymizeFeeDistribution){
        if(nAnonymizeFees > 0 && chainActive.Height() >= Params().GetConsensus().nMasternodePaymentsStartBlock){
            CAmount masternodePayment = GetMasternodePayment(chainActive.Height() + 1, 0) + nAnonymizeFees/10;
            FillBlockPayments(txNew, chainActive.Height() + 1, masternodePayment, pblock->txoutMasternode, pblock->voutSuperblock);

            for(int g = 2; g < 11; g++){
                CTxOut tempTx;
                mnpayments.FillBlockPayee(txNew, chainActive.Height() + g, nAnonymizeFees/10, tempTx);
            }
        }
        //no masternode fees
        else{
            if (chainActive.Height() >= Params().GetConsensus().nMasternodePaymentsStartBlock) {
                CAmount masternodePayment = GetMasternodePayment(chainActive.Height() + 1, 0);
                FillBlockPayments(txNew, chainActive.Height() + 1, masternodePayment, pblock->txoutMasternode, pblock->voutSuperblock);
            }
        }
    }
    //Utilize new distribuition model
        ...
        if(!GetMasternodeFeePayment(returnFee, payFees, block))
            return error("%s: GetMasternodeFeePayment failed.", __func__);

        //Pay node winner block reward
        CAmount masternodePayment = GetMasternodePayment(chainActive.Height() + 1, 0);
        FillBlockPayments(txNew, chainActive.Height() + 1, masternodePayment, pblock->txoutMasternode, pblock->voutSuperblock);

        //add current block fee since we skip it in GetMasternodeFeePayment()
        returnFee += nAnonymizeFees;

        //pay or dont pay the fees to all nodes
        if(payFees && returnFee != 0){
            vector<CMasternode> masternodeVector = mnodeman.GetFullMasternodeVector();

            int totalActiveNodes = 0;
            int startBlock = (chainActive.Height() + 1) - (Params().GetConsensus().nAnonymizeFeeDistributionCycle - 1);
            int64_t ensureNodeActiveBefore = chainActive[startBlock]->GetBlockTime();

            for(auto node: masternodeVector){
...
bool CWallet::SignBlock(CBlockTemplate *pblocktemplate, int nHeight, int64_t nSearchTime)
{
    ...
    if(chainActive.Height() + 1 > Params().GetConsensus().nMasternodePaymentsStartBlock){
        for(int i = 1; i < pblock->vtx.size(); i++){
            ...
            //Avoid 2-way shading miscalculation
            if(pblock->vtx[i]->IsZerocoinMint() && !pblock->vtx[i]->IsZerocoinSpend()){
                ...
                nAnonymizeFees += mintAmount * 0.0025;

            }
        }
    }
    if(nAnonymizeFees > nFees){
        LogPrintf("\nCWallet::SignBlock() ERROR: nAnonymizeFees not able to payout, reverting to nFees, nAnonymizeFees=%llf, nFees=%llf \n", nAnonymizeFees, nFees);
        nAnonymizeFees =  nFees;
        nFees = 0;
    }
    else
        nFees -= nAnonymizeFees;

    //LogPrintf("\nAnonymize Fees: nAnonymizeFees=%llf, nFees=%llf \n", nAnonymizeFees, nFees);
    ...
    if (CreateCoinStake(pblock->nBits, nSearchTime, nHeight, nFees, txCoinStake, key, pblocktemplate, nAnonymizeFees, commitment, witnessroot))
    {
...
bool CWallet::FindUnloadedAnonymizeTransactions(const CTransaction& tx)
{

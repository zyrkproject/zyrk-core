...
static map<uint256, int64_t> anonymizedCoins;
                              ...
                              CZerocoinTxInfo *zerocoinTxInfo)
{
    ...
    if(tx.IsZerocoinSpend()) {
        ...
        BOOST_FOREACH(const CTxOut &txout, tx.vout)
        {
            ...
            anonymizedCoins[hashTx] = tx.GetValueOut();
            i++;
        }
    }
...
void DisconnectTipAnonymize(CBlock & /*block*/, CBlockIndex *pindexDelete) {
 ...
 */
bool ConnectBlockAnonymize(CValidationState &state, const CChainParams &chainparams, CBlockIndex *pindexNew, const CBlock *pblock) {
...
uint64_t CZerocoinState::GetTotalZerocoins(){
    ...
    //total coins spent
    for(auto it = anonymizedCoins.begin(); it != anonymizedCoins.end(); it++) {
